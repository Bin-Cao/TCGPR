...
def dataset_eval(dataset,n_restarts_optimizer,alpha,normalize_y,exploit_model):
...
def best_supplement(up_data,ini_data,n_restarts_optimizer,alpha,normalize_y,exploit_model,sampling_cap,up_search,fea_num,single_samples=True):
    # add one datum 
    if single_samples == True:
        ...
        for i in range(len(up_data)):
            ...
            GGMF_inter,_Rvalue, _y_std_set= dataset_eval(dataset_inter,n_restarts_optimizer,alpha,normalize_y,exploit_model)
...
# the main function
def cal_TCGPR(filePath, initial_set_cap=3, sampling_cap=1, ratio=0.1, up_search = 2e2, exploit_coef=2, alpha=1e-10, n_restarts_optimizer=10,
    ...
    if type(initial_set_cap) != int:
        if type(initial_set_cap) == list:
            ...
            GGMF_ini,Rvalue,_ = dataset_eval(ini_data,n_restarts_optimizer,alpha,normalize_y,exploit_model)
        ...
        for set_index in range(len(initial_dataset_index)):  
            ...
            GGMF_ini,Rvalue,y_std_set = dataset_eval(ini_data,n_restarts_optimizer,alpha,normalize_y,exploit_model) 
    ...
    if type(sampling_cap) != int:
        ...
        print('The Sampling capacity at each iteration must be larger than 1!')
        print('Please input a right integer value')
    elif sampling_cap == 1:
        ...
        while True: 
            ...
            if len(up_data) < sampling_cap:
                ...
                _uncer_array,GGMF_inter_array,_Rvalue_set,_ = best_supplement(up_data,ini_data,n_restarts_optimizer,alpha,normalize_y,exploit_model,sampling_cap,up_search,fea_num,single_samples=True)
        ...
        while True: 
            ...
            if len(up_data) < sampling_cap:
                ...
                _uncer_array,GGMF_inter_array,_Rvalue_set,sampling_index = best_supplement(up_data,ini_data,n_restarts_optimizer,alpha,normalize_y,exploit_model,sampling_cap,up_search,fea_num,single_samples=0)
                Expected_improvement = Cal_EI(GGMF_inter_array, _uncer_array, GGMF_GLOBAL_Track[GGMF_inter_index],exploit_coef)
                EI_max = np.nanmax(Expected_improvement, axis=0)
                EI_index = np.where(Expected_improvement == EI_max)[0][0]
                fitting_goodness_track.append(1 - _Rvalue_set[EI_index])
